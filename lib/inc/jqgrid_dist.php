 ...
 * @version 2.2.5 build 20171031-2254
...
class jqgrid
{
	...
	function set_columns($cols = null, $change_field = false)
	{
		...
		if (!$cols)
		{
			foreach($f as $c)
			{
				...
				if (!empty($meta))
				{
					...
					{
						if ($this->options["autowidth"]) $col["width"] = "20";
					}	
		...
		for($i=0;$i<count($cols);$i++)
		{
			...
			if (isset($cols[$i]["stype"]) && $cols[$i]["stype"] == "select-multiple")
			{
				...
				$cols[$i]["searchoptions"]["attr"] = array("multiple"=>"multiple", "size"=>4, "defaultValue"=>$cols[$i]["searchoptions"]["defaultValue"]);
			}
			...
			if (isset($cols[$i]["formatter"]) && $cols[$i]["formatter"] == "autocomplete")
			{
				...
				{
					$cols[$i]["editrules"]["required"] = true;
					$force = 1;
				}
		...
		$out .= ",'loadComplete': function(ids) {";
				...
				$out .= "
					...
					if (get_querystring_value('form') == 'add')
					{
						...
						jQuery('#editmod{$grid_id} .CaptionTD').css('width','20%');
						jQuery('#editmod{$grid_id} .navButton').css('width','20%');
	...
	?>
		...
		// Toggle search bar
		// jQuery("#<?php echo $grid_id?>").jqGrid('navButtonAdd',"#<?php echo $grid_id."_pager"?>",{caption:'', title:'', buttonicon :'ui-icon-gear',
			// onClickButton:function(){
				// jQuery("#gbox_<?php echo $grid_id?> .ui-search-toolbar").toggle();
			// }
		// });
		<?php } ?>
		...
		jQuery("#<?php echo $grid_id?>").jqGrid('navButtonAdd',"#<?php echo $grid_id."_pager"?>",{caption:cols_text, title:select_cols_text, buttonicon :'ui-icon-note',
			onClickButton:function(){
				jQuery("#<?php echo $grid_id?>").jqGrid('columnChooser',{msel:'multipleSelect', msel_opts: {filter:false,maxHeight:145,keepOpen:true,isOpen:true}, width : 250, height:180, modal:true, done:function(){ c = jQuery('#colchooser_<?php echo $grid_id?> select').val(); var colModel = jQuery("#<?php echo $grid_id?>").jqGrid("getGridParam", "colModel"); str = ''; jQuery(c).each(function(i){ str += colModel[c[i]]['name'] + ","; }); document.cookie = 'jqgrid_colchooser_<?php echo $grid_id ?>=' + str; jQuery('#<?php echo $grid_id ?>').trigger('resize'); }, "dialog_opts" : {"minWidth": 270} });
		...
		function link_multiselect(elem)
		{
			...
			var $elem = jQuery(elem), id = elem.id,
				...
				options = {
					selectedList: 4,
					height: ((optLen>10)? jQuery(window).height() * 0.6 :"auto"),
					checkAllText: "All",
					uncheckAllText: "None",
			...
			setTimeout(function(){
				if(jQuery.ui)
				{
					if(jQuery.ui.datepicker)
					{
						...
						if (toolbar == 0)
						{
							jQuery(el)[0].style.setProperty('width', parseInt(jQuery(el).css('width'))-36+'px', 'important');
		...
		function link_upload(el,field)
		{
			setTimeout(function(){
				...
				if(jQuery(el).val() != '')
				{
					...
					jQuery(el).parent().append("<input style='display:block' class='reset_upload' type='button' value='"+reset_text+"' onclick='jQuery(\"#"+field+"\").val(\"\"); fx_replace_upload(\"#"+field+"\",\""+field+"\");' />");
					jQuery(el).hide();
				}
				else
				{
													...
													"change": function( event, ui )
															{
																...
																if ( !ui.item )
																{
																	...
																	if ( !valid ) {
																		...
																		jQuery('#FormError>td').html('<div>'+"&nbsp;Selection didn't match any item"+'</div>'); jQuery('#FormError').show().delay(3000).fadeOut();
							...
							else
							{
								...
								else
								{
									...
									if (!jQuery(sel).data('select2'))
										jQuery(sel).prepend("<option selected='selected' disabled='disabled' style='display:none'>Select...</option>");
		...
		fx_reload_dropdown = function (f,src)
		{
			// fill dropdown 1 based on row data
			o = jQuery('select[name='+src+']').get();
			fx_get_dropdown(o,f);
		}
					...
					success: function (data, status)
					{
						...
						if(typeof(data.error) != 'undefined')
						{
							...
							else
							{
								...
								jQuery(o).parent().append("<input style='display:block' class='reset_upload' type='button' value='"+reset_text+"' onclick='fx_replace_upload(\"#"+field+"\",\""+field+"\");' />");
		...
		if (!fx_fullscreen)
		{
			var fx_fullscreen = function () {
				...
				jQuery(".ui-jqgrid").each(function(i,o){
					...
					// remaining offset
					var h_offset = jQuery('.ui-jqgrid-titlebar').height() 
									+jQuery('.ui-jqgrid-hdiv').height()
									+jQuery('.ui-jqgrid-toppager').height()
									+jQuery('.ui-jqgrid-pager').height()+3;
	...
	{
		if (file_exists(dirname(__FILE__) . "/htmlpurifier/HTMLPurifier.standalone.php"))
		{
